...
async function callGemini(text: string): Promise<AiResult> {
...
async function callClaude(text: string, modelId: string): Promise<AiResult> {
...
async function callOpenAI(text: string, modelId: string): Promise<AiResult> {
...
export async function POST(req: NextRequest) {
  try {
    ...
    switch (model) {
      case 'gemini':        result = await callGemini(text); break;
      case 'claude-haiku':  result = await callClaude(text, 'claude-haiku-4-5-20251001'); break;
      case 'claude-sonnet': result = await callClaude(text, 'claude-sonnet-4-5'); break;
      case 'claude-fable':  result = await callClaude(text, 'claude-fable-5'); break;
      case 'gpt-4o-mini':   result = await callOpenAI(text, 'gpt-4o-mini'); break;
      case 'gpt-4o':        result = await callOpenAI(text, 'gpt-4o'); break;
      case 'gpt-4.5':       result = await callOpenAI(text, 'gpt-4.5-preview'); break;
      case 'gpt-5.6-sol':   result = await callOpenAI(text, 'gpt-5.6-sol'); break;
      case 'gpt-5.6-luna':  result = await callOpenAI(text, 'gpt-5.6-luna'); break;
      case 'gpt-5.6-terra': result = await callOpenAI(text, 'gpt-5.6-terra'); break;
      default:              result = await callGemini(text);
    }
    ...
    return NextResponse.json({ extracted, usage: result.usage });
  } catch (e) {
