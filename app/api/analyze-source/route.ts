...
type AnalyzeMode = 'source' | 'metrics' | 'discover' | 'chapter' | 'archive-split' | 'doc-import';
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (mode === 'archive-split') {
      ...
      console.log(`[archive-split] model=${model} pdf=${!!pdfBase64} 화수=${chapters.length}`);
      return NextResponse.json({ chapters, usage: splitResult.usage, usedModel: model });
    }
