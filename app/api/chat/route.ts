...
\`\`\`json
{
  "similarWorks": "유사 작품/경쟁작 — 반드시 실제로 존재하는 구체적인 작품 제목을 2개 이상 「」로 감싸 나열하고, 각 작품이 왜 유사한지 한 줄 근거를 붙이세요. 예: 「나 혼자만 레벨업」 — 약자가 각성해 성장하는 헌터물. 실존하지 않는 작품명은 절대 지어내지 마세요",
  "genreTrends": "장르 트렌드",
...
function summarizeArchive(archive?: OriginalArchive): string {
  ...
  for (const v of archive.volumes) {
    ...
    if (v.summary?.trim()) lines.push(`${vLabel} 전체 요약 — ${v.summary.trim()}`);
